...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        self.image_thresholded_publisher = self.create_publisher(Image, '/color_detection/thresholded_image', 10)
    ...
    def sync_callback(self, color_msg, depth_msg, info_msg):
        ...
        # 補正後の画像座標系における把持対象の位置を取得（2D）
        rect_point = camera_model.rectifyPoint(point)

        # カメラ座標系から見た把持対象の方向（Razy）を取得する
        ray = camera_model.projetctPixelTo3DRay(rect_point)

        # 把持対象までの距離を取得
        # 把持対象の表面より少し奥を掴むように設定
        DEPTH_OFFSET = 0.015
        cv_depth = self.bridge.imgmsg_to_cv2(depth_msg, desired_encoding=depth_msg.encoding)

        # カメラから把持対象の表面までの距離
        from_distance = cv_depth[int(point[1]), int(point[0])] / 1000.0
