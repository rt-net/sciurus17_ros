...
void init_reconfigure(  DXLPORT_CONTROL *driver )
{
    for( std::vector<JOINT_CONTROL>::iterator it=driver->joints.begin() ; it!=driver->joints.end() ; ++it ){
        ...
        dynamic_reconfigure::Server<sciurus17_msgs::ServoParameterConfig>::CallbackType f;
...
int main( int argc, char* argv[] )
{
    ...
    DXLPORT_CONTROL sciurus17( nhPrivate, setting );
    if( !sciurus17.get_init_stat() ){
        std::string errorlog;
        if( sciurus17.get_error( errorlog ) > 0 ){
            ROS_INFO("Initialize error (%s)",errorlog.c_str());
        }
        return -1;
    }
    controller_manager::ControllerManager cm( &sciurus17, nh );
    driver_addr = &sciurus17;

    ros::Publisher lasterror_pub = nhPrivate.advertise<std_msgs::String>("lasterror", 10);
    init_topics( &sciurus17, nhPrivate );
    ...
    ros::Time t = sciurus17.getTime();
    ros::Duration d = sciurus17.getDuration(t);
...
//    sciurus17.set_watchdog_all( MSEC2DXL_WATCHDOG(1000) ); //1秒の間,無通信で停止

    while( ros::ok() ){
        d = sciurus17.getDuration(t);
        t = sciurus17.getTime();

        if( sciurus17.tempCount != prev_tempCount ){
            read_temp_flg = true;
            prev_tempCount = sciurus17.tempCount;
        }else{
            read_temp_flg = false;
        }
        publish_topic_data( &sciurus17, read_temp_flg );

        sciurus17.lock_port();
        if( sciurus17.read( t, d ) ){
            cm.update( t, d );
            sciurus17.write( t, d );
        }
        sciurus17.unlock_port();
        ...
        std::string errorlog;
        while( sciurus17.get_error( errorlog ) > 0  ){    // error log check
            lasterror_out.data = errorlog;
            lasterror_pub.publish( lasterror_out );
        }

#ifdef REACTIVE_RATE_FUNCTION
        if( sciurus17.is_change_positions() ){
            rate_active.sleep();
        }else{
            rate_wait.sleep();
        }
